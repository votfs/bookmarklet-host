...
function getRelevantNumericValues() {
  ...
  try {
    elements.forEach(el => {
      ...
      const cleanedText = text.replace(/[^0-9.]/g, ''); // Remove non-numeric characters (e.g., " SOL")
    ...
    waitFor(confirmAndProceedToAddressInputSelector, (confirmProceedBtn) => {
      ...
      waitFor(addressInputSelector, (addressInputEl) => {
        ...
        waitFor(maxButtonSelector, (maxBtn) => {
          if (maxBtn.textContent && maxBtn.textContent.trim().toLowerCase() === 'drag me') { // Updated to 'drag me'
            maxBtn.click();
